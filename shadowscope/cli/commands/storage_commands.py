...
from shadowscope.core import storage, config
...
def stats():
    ...
    table.add_row("Total Targets", str(stats["targets"]))
    table.add_row("Total Results", str(stats["results"]))
    table.add_row("Total Modules", str(stats["modules"]))
    
    console.print(table)
    
    if stats["targets_by_type"]:
        console.print("\n[bold]Targets by Type:[/bold]")
        for target_type, count in sorted(stats["targets_by_type"].items(), key=lambda x: (-x[1], x[0])):
            console.print(f"  {target_type}: {count}")
    
    if stats["results_by_module"]:
        console.print("\n[bold]Results by Module:[/bold]")
        for module, count in sorted(stats["results_by_module"].items(), key=lambda x: (-x[1], x[0]))[:10]:
            console.print(f"  {module}: {count}")
    
    if stats["results_by_status"]:
        console.print("\n[bold]Results by Status:[/bold]")
        for status, count in sorted(stats["results_by_status"].items(), key=lambda x: (-x[1], x[0])):
...
@app.command()
def backup(
    path: Optional[str] = typer.Option(None, "--path", "-p", help="Backup file path (auto-generated if not specified)")
):
...
def export(
    ...
    if format != "json":
        console.print("[yellow]Only JSON format is currently supported for export[/yellow]")
        format = "json"
    
    output_path = storage.export_json(path, targets=targets, results=results, modules=modules)
    console.print(f"[green]+[/green] Exported data to {output_path}")
...
def clear(
    ...
    console.print(f"[yellow]![/yellow] Cleared data")
...
def optimize():
    ...
    # For SQLite, we can run some basic optimization
    try:
...
def check():
    ...
    try:
        ...
        stats = storage.get_stats()
        console.print(f"[green]+[/green] Database is accessible")
        console.print(f"  Targets: {stats['targets']}")
        console.print(f"  Results: {stats['results']}")
        console.print(f"  Modules: {stats['modules']}")
        
        # Check for orphaned results (results with non-existent targets)
        all_results = storage.get_results_by_module("")
